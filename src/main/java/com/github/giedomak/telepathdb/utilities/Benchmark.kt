...
import com.github.giedomak.telepathdb.kpathindex.utilities.AdvogatoImport
...
object Benchmark {
    ...
    private fun extendK() {
        ...
        KExtender.run(TelepathDB.kPathIndex, 2, true)
    }

    private fun setupLUBM() {
        TelepathDB.pathIdentifierStore.clear()
        Node.clear()
        // Re-use the index we already constructed
        TelepathDB.kPathIndex = KPathIndexDisk(dir = kPathIndexLocationLUBM)
        // The cardinality estimator needs to know this new index
        TelepathDB.cardinalityEstimation = SynopsisCardinalityEstimation(kPathIndex = TelepathDB.kPathIndex)

        // Make sure our Nodes and Paths are known in our PathIdentifierStore and NodeIdentifierStore
        LUBMImport.run(TelepathDB.kPathIndex, LUBMDataset, true)

        extendK()

        // Clear the results in our memory and cache
        TelepathDB.memoryManager.clear()
    }
    ...
    private fun setupAdvogato() {
        ...
        Node.clear()
        // Re-use the index we already constructed
        TelepathDB.kPathIndex = KPathIndexDisk(dir = kPathIndexLocationAdvogato)
        // The cardinality estimator needs to know this new index
        TelepathDB.cardinalityEstimation = SynopsisCardinalityEstimation(kPathIndex = TelepathDB.kPathIndex)

        // Make sure our Nodes and Paths are known in our PathIdentifierStore and NodeIdentifierStore
        AdvogatoImport.run(TelepathDB.kPathIndex, AdvogatoDataset, true)
    ...
    private fun runQuery(key: String, value: String, results: MutableList<Pair<String, String>>, first: Boolean): MutableList<Pair<String, String>> {
        ...
        for (i in 1..20) {
            ...
            val query = Query(TelepathDB, value)
